...
class EditOutcomePage extends StatefulWidget {
  EditOutcomePage({super.key, required this.outcome});

  TransactionModel outcome;
...
class _EditOutcomePageState extends State<EditOutcomePage> {
  ...
  @override
  void initState() {
    _trxDateCtrl.text = DateFormat('yyyy-MM-dd').format(widget.outcome.trxDate.toDate());
    _amountCtrl.text = widget.outcome.amount.toString();
    _descriptionCtrl.text = widget.outcome.description;
    super.initState();
  }
                                      ...
                                      if (_formAddKey.currentState!
                                        ...
                                        try {
                                          ...
                                          var data = TransactionModel(
                                            id: widget.outcome.id,
                                            amount: int.parse(_amountCtrl.text),
                                            description: _descriptionCtrl.text,
                                            isIncome: false,
                                            trxDate:
                                                Timestamp.fromDate(dateTimeTrx),
                                            userId: widget.outcome.userId,
                                          );

                                          debugPrint(data.toString());

                                          _outcomeBloc
                                              .add(UpdateOutcome(data: data));
